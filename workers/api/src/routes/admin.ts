...
admin.get('/teachers', async (c) => {
  ...
  const totalRow = await c.env.DB.prepare('SELECT COUNT(*) as n FROM teachers').first<{ n: number }>();
...
admin.post('/teachers', async (c) => {
  ...
  await c.env.DB.prepare('INSERT INTO teachers (id, email, password_hash) VALUES (?, ?, ?)')
    .bind(id, normalizedEmail, await hashPassword(password)).run();
...
admin.put('/teachers/:id', async (c) => {
  ...
  if (email) {
    ...
    await c.env.DB.prepare('UPDATE teachers SET email = ? WHERE id = ?')
      .bind(email.trim().toLowerCase(), id).run();
  }
  if (password) {
...
admin.delete('/teachers/:id', async (c) => {
  ...
  const taskRows = await c.env.DB.prepare('SELECT id FROM tasks WHERE teacher_id = ?')
    .bind(id).all<{ id: string }>();
  for (const t of taskRows.results ?? []) {
    await cleanupTask(c.env, t.id);
  }
